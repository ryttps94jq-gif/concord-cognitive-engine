...
namespace Concordia
{
    ...
    public class ConcordClient : MonoBehaviour
    {
        ...
        /// <summary>
        /// Persist the Unity WorldMemory slice to the kernel. Stock/need/caravans
        /// stay honestly empty on kingdom:request until this lands. Diseases are
        /// never included — they do not cross worlds.
        /// </summary>
        public Task SyncKingdom(WorldId id, LivingSaveRec all, WorldSliceRec slice)
        {
            if (!Connected) return Task.CompletedTask;
            worldId = WorldBook.Folder(id);
            var stock = slice != null ? slice.stock.ToString("0.###", System.Globalization.CultureInfo.InvariantCulture) : "null";
            var need = slice != null ? slice.need.ToString("0.###", System.Globalization.CultureInfo.InvariantCulture) : "null";
            var staple = slice != null ? Escape(slice.staple ?? "") : "";
            var imports = slice != null ? Escape(slice.imports ?? "") : "";
            var pop = slice != null ? slice.population.ToString() : "0";
            var caravans = Escape(all != null ? all.caravansCsv ?? "" : "");
            var tariffs = Escape(all != null ? all.tariffsCsv ?? "" : "");
            var plots = Escape(all != null ? all.plotsCsv ?? "" : "");
            var travelers = Escape(all != null ? all.travelersCsv ?? "" : "");
            return SendEvt("kingdom:sync",
                "{\"worldId\":\"" + Escape(worldId)
                + "\",\"enumId\":\"" + id
                + "\",\"stock\":" + stock
                + ",\"need\":" + need
                ...
                + "\",\"imports\":\"" + imports
                + "\",\"population\":" + pop
                + ",\"caravansCsv\":\"" + caravans
                + "\",\"tariffsCsv\":\"" + tariffs
                + "\",\"plotsCsv\":\"" + plots
                + "\",\"travelersCsv\":\"" + travelers
                + "\"}");
        }
