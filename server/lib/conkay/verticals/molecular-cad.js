...
export function parseMolecularIntent(textOrFormula) {
  ...
  if (peg || /\bPEG\b/i.test(raw)) {
    const n = Math.min(48, Math.max(4, parseInt(peg?.[1] || peg?.[2] || '12', 10) || 12));
  ...
  if (pe && !/glycol/i.test(raw)) {
    const n = Math.min(48, Math.max(4, parseInt(pe[1] || '16', 10) || 16));
  ...
  if (pla) {
    const n = Math.min(36, Math.max(4, parseInt(pla[1] || '10', 10) || 10));
  ...
  return {
    ok: false,
    error: 'unrecognized_intent — try H2O, C2H6O, water, PEG n=12, PE n=16, PLA n=8',
    code: 'NO_INTENT',
  };
}
...
function placeMolecule(tokens) {
  ...
  // Simple geometry: central heaviest non-H, hydrogens around, others on ring
  ...
  // Place heavy atoms on a ring / chain
  const R = 1.4;
  heavy.forEach((a, i) => {
    if (heavy.length === 1) {
      ...
      positions.push({ ...a, x: Math.cos(th) * R, y: 0, z: Math.sin(th) * R });
    }
  });
  ...
  if (heavy.length === 1) { /* no heavy-heavy */ }
  // Attach H around parent heavy (round-robin)
  hydro.forEach((h, hi) => {
...
function placePolymer(kind, n) {
  const atoms = [];
  const bonds = [];
  // Coarse bead chain — biocompatible polymer GEOMETRY only
