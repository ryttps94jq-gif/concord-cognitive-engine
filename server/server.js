...
async function runMacro(domain, name, input, ctx) {
  ...
  const safeReadBypass =
    _method === "GET" && (
      // DTU reads (home page + graph + export + council selection)
      (domain === "dtu" && (name === "list" || name === "get") && _path.startsWith("/api/dtus")) ||
      // Status pings
      (_path === "/api/status")
    );
  ...
  if (!safeReadBypass) {
    ...
    if (!c2.ok) {
      ...
      const safeReadBypass =
        reqMethod === "GET" &&
        (reqPath.startsWith("/api/status") || reqPath.startsWith("/api/dtus"));
...
async function latticeAutonomousTick() {
  try {
    ...
    try { kernelTick({ type: "AUTONOMOUS_TICK", meta: { source: "cron" } }); } catch {}
