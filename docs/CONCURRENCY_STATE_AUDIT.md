...
|---|---|---|---|
| `STATE.wallets` | `server.js:63512-63590` (`/api/credits/wallet\|earn\|spend`, `/api/credits/balance`) | **HIGH.** A separate in-memory credits wallet: `balance: 100` starting grant, `wallet.balance += amount` where `amount` + `reason` are **client-supplied**, no `economy_ledger` write, no persistence. Wired from `concord-frontend/lib/api/client.ts:880`. On a cluster your balance depends which node you hit; today it's already a free-mint surface + a fabricated balance (honesty-invariant concern independent of clustering). | Decide: delete (frontend `billing` helper stops calling it) or back it with `user_wallets`. Do not cluster until resolved. |
