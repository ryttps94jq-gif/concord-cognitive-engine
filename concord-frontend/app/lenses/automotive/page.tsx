...
import {
  Car, Wrench, ClipboardList, DollarSign, Users,
  Plus, Search, X, Trash2, BarChart3, CheckCircle2, FileText, Award, Calculator,
  Layers, ChevronDown, Receipt, Gauge, Fuel, Zap,
} from 'lucide-react';
...
const TRADE_MATERIALS = ['Motor Oil','Brake Pads','Filters','Spark Plugs','Belts','Hoses','Tires','Battery','Alternator','Starter','Coolant','Transmission Fluid'];
const TRADE_CERTS = ['ASE Master Tech','ASE Brakes','ASE Engine','ASE Electrical','ASE HVAC','State Inspection License','Hybrid/EV Certification'];

export default function AutomotiveLensPage() {
  useLensNav('automotive');
  const { latestData: realtimeData, isLive, lastUpdated, insights } = useRealtimeLens('automotive');
  ...
  const { items, isLoading, isError, error, refetch, create, update, remove } = useLensData<TradeArtifact>('automotive', activeArtifactType, { seed: [] });
  ...
  const renderLibrary = () => (
    <div className="space-y-3">
      ...
      {isLoading ? <div className="flex items-center justify-center py-12"><div className="w-6 h-6 border-2 border-red-500 border-t-transparent rounded-full animate-spin" /></div>
      : filtered.length === 0 ? <div className={cn(ds.panel, 'text-center py-12')}><Car className="w-12 h-12 text-gray-600 mx-auto mb-3" /><p className={ds.textMuted}>No {activeArtifactType} items yet</p><button onClick={openCreate} className={cn(ds.btnPrimary, 'mt-3')}><Plus className="w-4 h-4" /> Create First</button></div>
      : filtered.map((item, index) => {
        const d = item.data as unknown as TradeArtifact; const sc = STATUS_CONFIG[d.status] || STATUS_CONFIG.pending;
        return (<motion.div key={item.id} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.05 }} className={ds.panelHover} onClick={() => openEdit(item)}><div className="flex items-center justify-between"><div className="flex items-center gap-3"><Car className="w-5 h-5 text-neon-cyan" /><div><p className="text-white font-medium">{d.name || item.title}</p><p className={ds.textMuted}>{d.client || ''} {d.address ? `- ${d.address}` : ''}</p></div></div><div className="flex items-center gap-2">{(d.totalCost || d.amount) && <span className="text-xs text-green-400">${(d.totalCost || d.amount || 0).toLocaleString()}</span>}<span className={`text-xs px-2 py-0.5 rounded-full bg-${sc.color}/20 text-${sc.color}`}>{sc.label}</span><button onClick={e => { e.stopPropagation(); handleAction('analyze', item.id); }} className={ds.btnGhost}><Zap className="w-4 h-4 text-neon-cyan" /></button><button onClick={e => { e.stopPropagation(); remove(item.id); }} className={ds.btnGhost}><Trash2 className="w-4 h-4 text-red-400" /></button></div></div></motion.div>);
      })}
    </div>
  );

  return (
    <div data-lens-theme="automotive" className="space-y-6 p-6">
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-slate-600 to-zinc-700 flex items-center justify-center"><Car className="w-5 h-5 text-white" /></div>
          <div><div className="flex items-center gap-2"><h1 className={ds.heading1}>Automotive</h1><LiveIndicator isLive={isLive} lastUpdated={lastUpdated} /></div><p className={ds.textMuted}>Jobs, estimates, codes, materials, CRM, invoicing, inspections, and certifications</p></div>
        </div>
        <div className="flex items-center gap-2">{runAction.isPending && <span className="text-xs text-neon-cyan animate-pulse">AI processing...</span>}<DTUExportButton domain="automotive" data={{}} compact /><button onClick={() => setShowDashboard(!showDashboard)} className={cn(showDashboard ? ds.btnPrimary : ds.btnSecondary)}><BarChart3 className="w-4 h-4" /> Dashboard</button></div>
      </header>
      <RealtimeDataPanel domain="automotive" data={realtimeData} isLive={isLive} lastUpdated={lastUpdated} insights={insights} compact />
        ...
        return (
          <div className="grid grid-cols-3 gap-4">
            <div className="p-3 bg-lattice-elevated rounded-lg border border-lattice-border flex items-center gap-3">
              ...
              <div><p className="text-lg font-bold text-white">{items.length}</p><p className="text-xs text-gray-400">Total Vehicles</p></div>
            </div>
            <div className="p-3 bg-lattice-elevated rounded-lg border border-lattice-border flex items-center gap-3">
              <Gauge className="w-5 h-5 text-yellow-400" />
              <div><p className="text-lg font-bold text-white">{serviceDue}</p><p className="text-xs text-gray-400">Service Due</p></div>
            </div>
            <div className="p-3 bg-lattice-elevated rounded-lg border border-lattice-border flex items-center gap-3">
              <Fuel className="w-5 h-5 text-green-400" />
              <div><p className="text-lg font-bold text-white">${totalRevenue.toLocaleString()}</p><p className="text-xs text-gray-400">Total Revenue</p></div>
            </div>
          </div>
        );
      })()}
