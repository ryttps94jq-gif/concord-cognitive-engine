...
export default function SpaceLensPage() {
  ...
  const stats = useMemo(() => ({
    activeMissions: missions.filter(m => ['active', 'orbit'].includes((m.data as MissionData).status)).length,
    totalMissions: missions.length,
    opSatellites: satellites.filter(s => (s.data as SatelliteData).status === 'operational').length,
