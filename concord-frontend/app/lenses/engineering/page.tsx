...
export default function EngineeringLensPage() {
  ...
  const {
    ...
    update,
    remove,
  } = useLensData<Record<string, unknown>>('engineering', 'project', { seed: [] });
  const { items: specItems, create: createSpec } = useLensData<Record<string, unknown>>(
    'engineering',
    'specification',
    { seed: [] }
  );
  ...
  const projects = projectItems.map((i) => ({
    ...
    title: i.title,
    ...(i.data || {}),
  })) as unknown as (EngineeringProject & { id: string; title: string })[];
  ...
  return (
    <LensPageShell
      ...
      description="Project management, specifications, and engineering standards"
      headerIcon={<Wrench className="w-6 h-6" />}
      isLoading={isLoading}
      isError={isError}
      error={error}
      onRetry={refetch}
    ...
    >
            ...
            {(
              ...
              return (
                <div key={phase} className="flex items-center">
                  <div
                    className={`px-3 py-2 rounded-lg text-center min-w-[90px] ${count > 0 ? 'bg-neon-cyan/10 border border-neon-cyan/30' : 'bg-lattice-surface border border-lattice-border'}`}
                  >
                    <p
                      className={`text-lg font-bold ${count > 0 ? 'text-neon-cyan' : 'text-gray-600'}`}
                    >
                      {count}
                    </p>
                ...
                >
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded bg-white/10 ${STATUS_COLORS[proj.status as ProjectStatus] || 'text-gray-400'}`}
                      >
        ...
        <div className="space-y-4">
          <div className="panel p-4">
            <h3 className="font-semibold mb-3 flex items-center gap-2">
              <FileText className="w-4 h-4 text-neon-cyan" /> Specifications Library
            </h3>
            <button
              onClick={() =>
                createSpec({
                  title: 'New Specification',
                  data: { type: 'specification', content: '' },
                })
              }
              className="px-3 py-1.5 bg-neon-cyan/20 text-neon-cyan rounded-lg text-sm hover:bg-neon-cyan/30 flex items-center gap-1.5"
            >
      ...
      {activeTab === 'standards' && (
        <div className="panel p-4">
          <h3 className="font-semibold mb-3 flex items-center gap-2">
            <CheckCircle className="w-4 h-4 text-green-400" /> Engineering Standards
          </h3>
          <div className="space-y-2">
            {[
