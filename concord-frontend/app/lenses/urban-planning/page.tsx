...
const STATUS_COLORS: Record<string, string> = {
  proposed: 'text-blue-400 bg-blue-400/10', approved: 'text-green-400 bg-green-400/10',
  in_progress: 'text-yellow-400 bg-yellow-400/10', completed: 'text-gray-400 bg-gray-400/10',
  rejected: 'text-red-400 bg-red-400/10', active: 'text-green-400 bg-green-400/10',
  amendment_pending: 'text-orange-400 bg-orange-400/10', variance_granted: 'text-purple-400 bg-purple-400/10',
  excellent: 'text-green-400 bg-green-400/10', good: 'text-blue-400 bg-blue-400/10',
  fair: 'text-yellow-400 bg-yellow-400/10', poor: 'text-orange-400 bg-orange-400/10',
...
export default function UrbanPlanningLensPage() {
  ...
  const { items, isLoading, isError, error, refetch, create, remove } =
    useLensData<ArtifactDataUnion>('urban-planning', currentType, { search: searchQuery || undefined });
  ...
  const stats = useMemo(() => ({
    ...
    criticalInfra: infra.filter(i => ['poor', 'critical'].includes((i.data as InfraData).condition)).length,
    totalInfra: infra.length,
  }), [projects, infra]);

  if (isLoading) {
    return (
      <div data-lens-theme="urban-planning" className="flex items-center justify-center h-full p-8">
        <div className="text-center space-y-3">
          <div className="w-8 h-8 border-2 border-emerald-500 border-t-transparent rounded-full animate-spin mx-auto" />
          <p className="text-sm text-gray-400">Loading urban planning data...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div data-lens-theme="urban-planning" className={cn(ds.pageContainer, 'space-y-4')}>
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-white">Urban Planning</h1>
            <p className="text-sm text-gray-400">Projects, zoning, infrastructure & transit planning</p>
          </div>
        </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <button onClick={() => create({ title: `New ${currentType}`, data: {} })} className="flex items-center gap-2 px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-sm">
      ...
      <div className="border-t border-white/10">
        ...
        {showFeatures && <div className="px-4 pb-4"><LensFeaturePanel lensId="urban-planning" /></div>}
      </div>
    </div>
  );
}
