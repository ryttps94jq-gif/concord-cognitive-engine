...
interface TradeArtifact {
  ...
  amperage?: number; voltage?: number; circuitType?: string;
}
...
const ELECTRICAL_MATERIALS = ['Romex 14/2', 'Romex 12/2', 'Romex 10/3', 'THHN Wire', 'MC Cable', 'Conduit EMT', 'Conduit PVC', 'Panel Box', 'Breakers', 'Outlets/Receptacles', 'Switches', 'LED Fixtures', 'Junction Boxes', 'Wire Nuts', 'GFCIs', 'AFCI Breakers'];
const ELECTRICAL_CERTS = ['Master Electrician', 'Journeyman Electrician', 'Apprentice Electrician', 'Low Voltage License', 'Fire Alarm Certification', 'Solar PV Installer'];

export default function ElectricalLensPage() {
  useLensNav('electrical');
  const { latestData: realtimeData, isLive, lastUpdated, insights } = useRealtimeLens('electrical');
        ...
        <div className={cn(ds.panel, 'w-full max-w-lg max-h-[85vh] overflow-y-auto')} onClick={e => e.stopPropagation()}>
          ...
          <div className="space-y-3">
            ...
            {activeArtifactType === 'Material' && (
              <><div><label className={ds.label}>Material</label><select className={ds.select} value={formMaterial} onChange={e => setFormMaterial(e.target.value)}>{ELECTRICAL_MATERIALS.map(m => <option key={m} value={m}>{m}</option>)}</select></div>
            ...
            {activeArtifactType === 'Certification' && (<div><label className={ds.label}>Certification</label><select className={ds.select} value={formCertType} onChange={e => setFormCertType(e.target.value)}>{ELECTRICAL_CERTS.map(c => <option key={c} value={c}>{c}</option>)}</select></div>)}
  ...
  return (
    <div data-lens-theme="electrical" className="space-y-6 p-6">
      <header className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-yellow-500 to-amber-600 flex items-center justify-center"><Zap className="w-5 h-5 text-white" /></div>
          <div><div className="flex items-center gap-2"><h1 className={ds.heading1}>Electrical</h1><LiveIndicator isLive={isLive} lastUpdated={lastUpdated} /></div><p className={ds.textMuted}>Jobs, estimates, NEC codes, materials, CRM, invoicing, inspections, and certifications</p></div>
        </div>
        <div className="flex items-center gap-2">{runAction.isPending && <span className="text-xs text-neon-cyan animate-pulse">AI processing...</span>}<DTUExportButton domain="electrical" data={{}} compact /><button onClick={() => setShowDashboard(!showDashboard)} className={cn(showDashboard ? ds.btnPrimary : ds.btnSecondary)}><BarChart3 className="w-4 h-4" /> Dashboard</button></div>
      </header>
      ...
      <div className="border-t border-white/10">
        ...
        {showFeatures && <div className="px-4 pb-4"><LensFeaturePanel lensId="electrical" /></div>}
      </div>
    </div>
  );
}
