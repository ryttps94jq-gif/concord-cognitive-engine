...
export default function FinanceLensPage() {
  ...
  const totalPnl = MOCK_ASSETS.reduce((sum, a) => sum + a.pnl, 0);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="lens-card">
          ...
          <p className="text-xl font-bold">{MOCK_ASSETS.sort((a, b) => b.pnlPercent - a.pnlPercent)[0].symbol}</p>
          <p className="text-green-400 text-sm">+{MOCK_ASSETS.sort((a, b) => b.pnlPercent - a.pnlPercent)[0].pnlPercent.toFixed(2)}%</p>
        </div>
        ...
        <div className="lens-card">
          ...
          <p className="text-xl font-bold">
            {formatCurrency(MOCK_ASSETS.reduce((sum, a) => sum + a.volume24h, 0), true)}
          </p>
        ...
        <div className="lens-card">
          ...
          <p className="text-xl font-bold">{MOCK_ALERTS.filter(a => a.active).length}</p>
          <p className="text-gray-400 text-sm">{MOCK_ORDERS.filter(o => o.status === 'open').length} open orders</p>
        </div>
      </div>
        ...
        <div className="panel p-4">
          ...
          <div className="relative w-full aspect-square max-w-[200px] mx-auto mb-6">
            <svg viewBox="0 0 100 100" className="w-full h-full -rotate-90">
              {(() => {
                let offset = 0;
                return MOCK_ASSETS.map((asset, i) => {
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <div className="text-center">
                <p className="text-2xl font-bold">{MOCK_ASSETS.length}</p>
  ...
  const renderTrade = () => {
    const selectedTradingAsset = MOCK_ASSETS.find(a => a.symbol === tradeAsset) || MOCK_ASSETS[0];
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {MOCK_ALERTS.map((alert) => {
          const asset = MOCK_ASSETS.find(a => a.symbol === alert.symbol);
