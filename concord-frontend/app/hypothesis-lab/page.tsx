...
const STATUS_CONFIG: Record<HypothesisStatus, { label: string; color: string; bgColor: string; borderColor: string; icon: typeof FlaskConical }> = {
  proposed:  { label: 'Proposed',  color: 'text-yellow-400', bgColor: 'bg-yellow-400/15', borderColor: 'border-yellow-400/30', icon: Lightbulb },
  testing:   { label: 'Testing',   color: 'text-blue-400',   bgColor: 'bg-blue-400/15',   borderColor: 'border-blue-400/30',   icon: Beaker },
  confirmed: { label: 'Confirmed', color: 'text-green-400',  bgColor: 'bg-green-400/15',  borderColor: 'border-green-400/30',  icon: CheckCircle2 },
  rejected:  { label: 'Rejected',  color: 'text-red-400',    bgColor: 'bg-red-400/15',    borderColor: 'border-red-400/30',    icon: XCircle },
  refined:   { label: 'Refined',   color: 'text-purple-400', bgColor: 'bg-purple-400/15', borderColor: 'border-purple-400/30', icon: Edit3 },
  archived:  { label: 'Archived',  color: 'text-gray-400',   bgColor: 'bg-gray-400/15',   borderColor: 'border-gray-400/30',   icon: Archive },
};

const ALL_STATUSES: HypothesisStatus[] = ['proposed', 'testing', 'confirmed', 'rejected', 'refined', 'archived'];
...
function decree(action: string, target?: string, data?: Record<string, unknown>) {
  return api.post('/api/sovereign/decree', { action, target, data }).then(r => r.data);
}
...
function StatusBadge({ status }: { status: HypothesisStatus }) {
  ...
  const Icon = cfg.icon;
  return (
    <span className={cn('inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border', cfg.bgColor, cfg.color, cfg.borderColor)}>
...
function ConfidenceMeter({ value, size = 'md' }: { value: number; size?: 'sm' | 'md' | 'lg' }) {
  ...
  const barColor =
    ...
    pct >= 20 ? 'from-orange-500 to-yellow-500' :
  ...
  return (
    <div className="flex items-center gap-2 w-full">
      ...
      <span className={cn('font-mono font-semibold tabular-nums', size === 'sm' ? 'text-[10px]' : 'text-xs',
    ...
    <div className={cn(
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 mt-1">
          <span className="text-[10px] text-gray-500">Weight: {(evidence.weight * 100).toFixed(0)}%</span>
          {evidence.dtuId && (
            <span className="text-[10px] text-gray-500 font-mono">DTU: {evidence.dtuId.slice(0, 8)}</span>
          )}
          {evidence.addedAt && (
            <span className="text-[10px] text-gray-500">{formatRelativeTime(evidence.addedAt)}</span>
          )}
        </div>
      </div>
    </div>
...
function TestResultCard({ test }: { test: TestResult }) {
  const resultConfig = {
    ...
    inconclusive: { color: 'text-yellow-400', bg: 'bg-yellow-400/10', icon: AlertTriangle, label: 'Inconclusive' },
  };
  const cfg = test.result ? resultConfig[test.result] : null;
  const Icon = cfg?.icon || Clock;

  return (
    <div className={cn('flex items-center gap-3 p-2.5 rounded-lg border border-lattice-border/50', cfg?.bg || 'bg-white/5')}>
      ...
      <span className={cn('text-[10px] font-medium px-2 py-0.5 rounded-full', cfg?.color || 'text-gray-400', cfg?.bg || 'bg-white/5')}>
        {cfg?.label || 'Pending'}
      </span>
    </div>
  );
...
function TimelineView({ events }: { events: TimelineEvent[] }) {
  if (!events || events.length === 0) {
    return <p className="text-xs text-gray-500 text-center py-4">No lifecycle events recorded yet.</p>;
  }
          ...
          <div key={i} className="relative">
            ...
            <div className="pl-2">
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-[10px] text-gray-500">{formatRelativeTime(evt.timestamp)}</span>
              </div>
              {evt.detail && (
                <p className="text-[11px] text-gray-400 mt-0.5">{evt.detail}</p>
              )}
            </div>
          </div>
        ))}
...
function Modal({ open, onClose, title, children, maxWidth = 'max-w-lg' }: {
  ...
  return (
    <div className={ds.modalContainer} onClick={onClose}>
      ...
      <div className={cn(ds.modalPanel, maxWidth, 'relative z-10')} onClick={e => e.stopPropagation()}>
...
export default function HypothesisLabPage() {
  ...
  const rejectMutation = useMutation({
    mutationFn: (reason: string) =>
      decree('hypothesis-reject', selectedId!, { reason }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['hypothesis-lab'] });
  ...
  const refineMutation = useMutation({
    mutationFn: (statement: string) =>
      decree('hypothesis-refine', selectedId!, { statement }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['hypothesis-lab'] });
  ...
  const selectedHypothesis: Hypothesis | null = useMemo(() => {
    ...
    if (fromDetail && typeof fromDetail === 'object' && 'id' in fromDetail) return fromDetail as Hypothesis;
    return hypotheses.find(h => h.id === selectedId) || null;
  }, [selectedId, detailData, hypotheses]);

  const domains = useMemo(() => {
    const set = new Set<string>();
    hypotheses.forEach(h => { if (h.domain) set.add(h.domain); });
  ...
  const stats = useMemo(() => ({
    ...
    avgConfidence: hypotheses.length > 0
  ...
  return (
    <div className="min-h-screen bg-lattice-void">
        ...
        <div className="max-w-[1600px] mx-auto px-4 lg:px-6 py-6">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className={cn(ds.textMuted, 'mt-0.5')}>
                  System 13a -- Formulate, test, and validate hypotheses through evidence-driven reasoning
                </p>
              </div>
            </div>
            <button
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mt-6">
            {[
              ...
              { icon: Lightbulb, label: 'Proposed', value: stats.proposed, color: 'text-yellow-400' },
              { icon: Beaker, label: 'Testing', value: stats.testing, color: 'text-blue-400' },
              { icon: CheckCircle2, label: 'Confirmed', value: stats.confirmed, color: 'text-green-400' },
              { icon: XCircle, label: 'Rejected', value: stats.rejected, color: 'text-red-400' },
              { icon: BarChart3, label: 'Avg Confidence', value: `${(stats.avgConfidence * 100).toFixed(0)}%`, color: 'text-cyan-400' },
            ].map(stat => (
      ...
      <div className="max-w-[1600px] mx-auto px-4 lg:px-6 py-4 space-y-3">
        ...
        {showFilters && (
          <div className={cn(ds.panel, 'flex flex-wrap items-end gap-4')}>
            ...
            <div className="space-y-1">
              <label className={ds.label}>Min Confidence: {(confidenceRange[0] * 100).toFixed(0)}%</label>
              <input
                type="range"
                min="0"
                max="1"
            ...
            <div className="space-y-1">
              <label className={ds.label}>Max Confidence: {(confidenceRange[1] * 100).toFixed(0)}%</label>
              <input
                type="range"
                min="0"
                max="1"
            ...
            {!isListLoading && filteredHypotheses.length === 0 && (
              <div className={cn(ds.panel, 'text-center py-16')}>
                ...
                <p className="text-gray-400 mb-2">
                  {hypotheses.length === 0 ? 'No hypotheses yet' : 'No hypotheses match your filters'}
                </p>
            ...
            {selectedHypothesis ? (
              <div className={cn(ds.panel, 'space-y-5')}>
                ...
                <div>
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <h2 className={cn(ds.heading2, 'leading-snug')}>{selectedHypothesis.statement}</h2>
                  ...
                  {/* Rejection Reason */}
                  {selectedHypothesis.status === 'rejected' && selectedHypothesis.rejectionReason && (
                    <div className="mt-3 p-3 rounded-lg bg-red-500/10 border border-red-500/20">
                      <p className="text-xs text-red-400 font-medium">Rejection Reason</p>
                      <p className="text-sm text-gray-300 mt-1">{selectedHypothesis.rejectionReason}</p>
                    </div>
                  )}
                  ...
                  {selectedHypothesis.refinedFrom && (
                    <div className="mt-3 flex items-center gap-2 text-xs text-purple-400">
                      ...
                      Refined from: <span className="font-mono text-gray-400">{selectedHypothesis.refinedFrom.slice(0, 12)}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-wrap gap-2 border-t border-lattice-border pt-4">
                  ...
                  {selectedHypothesis.status !== 'confirmed' && selectedHypothesis.status !== 'archived' && (
                    <button
                      onClick={() => confirmMutation.mutate()}
                      disabled={confirmMutation.isPending}
                      className={cn(ds.btnBase, 'px-4 py-2 bg-green-500/20 text-green-400 hover:bg-green-500/30 border border-green-500/30')}
                    >
                      {confirmMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
                      Confirm
                    </button>
                  )}
                  {selectedHypothesis.status !== 'rejected' && selectedHypothesis.status !== 'archived' && (
                    <button
                  ...
                  {selectedHypothesis.status !== 'archived' && (
                    <button
                      ...
                      className={cn(ds.btnBase, 'px-4 py-2 bg-purple-500/20 text-purple-400 hover:bg-purple-500/30 border border-purple-500/30')}
                    >
                ...
                <div className="flex gap-1 border-b border-lattice-border">
                  {[
                    { key: 'evidence' as const, label: 'Evidence', icon: ThumbsUp, count: (selectedHypothesis.evidenceFor?.length || 0) + (selectedHypothesis.evidenceAgainst?.length || 0) },
                    { key: 'tests' as const, label: 'Tests', icon: Beaker, count: selectedHypothesis.tests?.length || 0 },
                    { key: 'timeline' as const, label: 'Timeline', icon: Activity, count: selectedHypothesis.timeline?.length || 0 },
                  ].map(tab => (
                    <button
                      key={tab.key}
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex items-center gap-1.5 text-xs">
                          ...
                          <span className="text-green-400 font-medium">{selectedHypothesis.evidenceFor?.length || 0} For</span>
                        </div>
                        ...
                        <div className="flex items-center gap-1.5 text-xs">
                          <span className="text-red-400 font-medium">{selectedHypothesis.evidenceAgainst?.length || 0} Against</span>
                      ...
                      <div className="flex gap-1">
                        {(['both', 'for', 'against'] as const).map(v => (
                          ...
                          >
                            {v === 'both' ? 'All Evidence' : v === 'for' ? 'Supporting' : 'Contradicting'}
                          </button>
                        ))}
                      </div>
                      ...
                      {(expandedEvidence === 'both' || expandedEvidence === 'for') && (
                        <div className="space-y-2">
                          ...
                          {(selectedHypothesis.evidenceFor || []).length === 0 ? (
                            <p className="text-xs text-gray-500 py-2">No supporting evidence yet.</p>
                          ) : (
                      ...
                      {(expandedEvidence === 'both' || expandedEvidence === 'against') && (
                        <div className="space-y-2">
                          ...
                          {(selectedHypothesis.evidenceAgainst || []).length === 0 ? (
                            <p className="text-xs text-gray-500 py-2">No contradicting evidence yet.</p>
                          ) : (
              ...
              <div className={cn(ds.panel, 'flex flex-col items-center justify-center py-20')}>
                ...
                <p className="text-sm text-gray-600 mt-1 max-w-sm text-center">
                  Choose a hypothesis from the list to view its evidence, test results, and lifecycle timeline.
                </p>
              </div>
            )}
          </div>
      ...
      <Modal open={showCreateModal} onClose={() => setShowCreateModal(false)} title="New Hypothesis">
        <div className="space-y-4">
          <div>
      ...
      {/* Add Evidence Modal */}
      <Modal open={showEvidenceModal} onClose={() => setShowEvidenceModal(false)} title="Add Evidence">
        <div className="space-y-4">
          <div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => setShowEvidenceModal(false)} className={ds.btnGhost}>Cancel</button>
            <button
          ...
          {addEvidenceMutation.isError && (
            <p className="text-xs text-red-400">{(addEvidenceMutation.error as Error)?.message || 'Failed to add evidence'}</p>
          )}
        </div>
      </Modal>

      {/* Record Test Modal */}
      <Modal open={showTestModal} onClose={() => setShowTestModal(false)} title="Record Test Result">
        <div className="space-y-4">
          <div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => setShowTestModal(false)} className={ds.btnGhost}>Cancel</button>
            <button
          ...
          {updateTestMutation.isError && (
            <p className="text-xs text-red-400">{(updateTestMutation.error as Error)?.message || 'Failed to record test'}</p>
          )}
        </div>
      </Modal>

      {/* Reject Modal */}
      <Modal open={showRejectModal} onClose={() => setShowRejectModal(false)} title="Reject Hypothesis">
        <div className="space-y-4">
          <p className="text-sm text-gray-300">
            Provide a reason for rejecting this hypothesis. This helps document the reasoning process.
          </p>
          <div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => setShowRejectModal(false)} className={ds.btnGhost}>Cancel</button>
            <button
          ...
          {rejectMutation.isError && (
            <p className="text-xs text-red-400">{(rejectMutation.error as Error)?.message || 'Failed to reject hypothesis'}</p>
          )}
        </div>
      </Modal>

      {/* Refine Modal */}
      <Modal open={showRefineModal} onClose={() => setShowRefineModal(false)} title="Refine Hypothesis">
        <div className="space-y-4">
          <p className="text-sm text-gray-300">
            Refine the hypothesis statement based on new evidence or insights. The original will be preserved as a parent reference.
          </p>
          <div>
          ...
          <div className="flex justify-end gap-2 pt-2">
            <button onClick={() => setShowRefineModal(false)} className={ds.btnGhost}>Cancel</button>
            <button
          ...
          {refineMutation.isError && (
            <p className="text-xs text-red-400">{(refineMutation.error as Error)?.message || 'Failed to refine hypothesis'}</p>
          )}
        </div>
      </Modal>
    </div>
