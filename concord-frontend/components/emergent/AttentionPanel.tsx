...
export function AttentionPanel() {
  ...
  const [focusOverride, setFocusOverride] = useState<{ domain: string; weight: number; expiresAt: string } | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    apiHelpers.attentionAlloc.status().then((resp) => {
    ...
    const handler = (data: { allocation: AllocationEntry[]; focusOverride: typeof focusOverride }) => {
    ...
    return () => { socket.off('attention:allocation', handler); };
  }, []);
  ...
  return (
    <div className="panel p-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        {focusOverride && (
          <button onClick={unfocus} className="text-xs text-red-400 flex items-center gap-1 hover:underline">
      ...
      {focusOverride && (
        <div className="bg-yellow-500/10 border border-yellow-500/30 rounded p-2 text-xs text-yellow-400 flex items-center gap-2">
          ...
          Focus override: <strong>{focusOverride.domain}</strong> @ {(focusOverride.weight * 100).toFixed(0)}%
        </div>
      )}
