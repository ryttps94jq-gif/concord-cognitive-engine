...
export function ArtifactDeskPanel() {
  const [activeTab, setActiveTab] = useState<ModeTab>('Dashboard');

  const searchInputRef = useRef<HTMLInputElement>(null);
  useLensCommand(
    [
      { id: 'desk-listings', keys: 'shift+l', description: 'Desk listings', category: 'navigation', action: () => setActiveTab('Listings') },
      { id: 'desk-transactions', keys: 'shift+t', description: 'Desk transactions', category: 'navigation', action: () => setActiveTab('Transactions') },
      { id: 'desk-search', keys: '/', description: 'Focus desk search', category: 'navigation', action: () => searchInputRef.current?.focus() },
    ],
    { lensId: 'realestate' }
  );
    ...
    <div className="space-y-4">
      ...
      <header className={ds.sectionHeader}>
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className={ds.heading2}>Agent desk</h2>
            <p className={ds.textMuted}>
              Pipeline, rentals, showings &amp; investment records
            </p>
          </div>
        </div>
