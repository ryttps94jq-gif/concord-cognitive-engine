...
/**
 * FieldOpsPanel — EQuIS / ArcGIS Field Maps-shaped survey desk.
 * Artifact CRUD for Site / Species / EnvironmentalSample / TrailAsset /
 * WasteStream / ComplianceRecord / CarbonEntry / ResourceMetric /
 * SustainabilityGoal, plus the site map. Parent owns the view union;
 * this panel only mounts for a single `kind`.
 */
...
export type FieldOpsKind = ModeTab;
...
export function FieldOpsPanel({
  ...
  const [view, setView] = useState<'library' | 'dashboard'>(initialView);
  ...
  return (
    <LensPageShell
      domain="environment"
      title={MODE_TABS.find((t) => t.id === mode)?.label || 'Field records'}
      description="EQuIS-style field survey desk — sites, species, samples, trails, waste, permits"
