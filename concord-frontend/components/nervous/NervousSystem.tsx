...
export function NervousSystem({ className }: { className?: string }) {
  const [expanded, setExpanded] = useState(false);
  const [activeTab, setActiveTab] = useState<'pulse' | 'circuits' | 'events' | 'traces' | 'integrity' | 'wisdom'>('pulse');
    ...
    <div className={cn('bg-lattice-surface border border-lattice-border rounded-xl overflow-hidden', className)}>
      ...
      <div className="flex items-center justify-between p-4 border-b border-lattice-border">
        <div className="flex items-center gap-3">
          <div className={cn(
            'p-2 rounded-lg',
            overallStatus === 'healthy' ? 'bg-green-500/20' : overallStatus === 'degraded' ? 'bg-yellow-500/20' : 'bg-red-500/20'
          )}>
        ...
        {(Object.entries(pulseData?.components || {}) as [string, { status: string; score: number }][]).map(([name, comp]) => {
          ...
          return (
            <div key={name} className="text-center">
              <Icon className={cn('w-4 h-4 mx-auto', STATUS_COLORS[comp.status] || 'text-gray-400')} />
        ...
        {expanded && (
          ...
          >
            ...
            <div className="px-4 py-2 border-t border-lattice-border flex items-center gap-2 overflow-x-auto">
              {(['pulse', 'circuits', 'events', 'traces', 'integrity', 'wisdom'] as const).map(tab => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={cn(
                    'px-2 py-1 text-xs rounded-lg whitespace-nowrap capitalize transition-colors',
                    activeTab === tab ? 'bg-neon-cyan/20 text-neon-cyan' : 'text-gray-400 hover:text-white'
                  )}
                >
                  {tab}
                </button>
                ...
                <div className="space-y-2">
                  {(Object.entries(pulseData.components) as [string, { status: string; score: number }][]).map(([name, comp]) => (
                    <div key={name} className="p-2 bg-lattice-deep rounded-lg flex items-center justify-between">
                      ...
                      <span className={cn('text-xs', STATUS_COLORS[comp.status])}>{comp.score}/100</span>
                    </div>
                  ))}
                </div>
              )}
                    ...
                    <div key={name} className="p-2 bg-lattice-deep rounded-lg">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-white">{name}</span>
                          <span className={cn('text-xs capitalize', STATUS_COLORS[breaker.state])}>{breaker.state}</span>
                        </div>
                        {breaker.state === 'open' && (
                          <button
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    <div className="flex gap-3 text-xs">
                      <span className="text-red-400">Critical: {integrityData.bySeverity?.critical || 0}</span>
                      <span className="text-yellow-400">Error: {integrityData.bySeverity?.error || 0}</span>
                      <span className="text-gray-400">Warning: {integrityData.bySeverity?.warning || 0}</span>
                    </div>
                    {(integrityData.autofixable || 0) > 0 && (
                      ...
                      >
                        {fixIntegrity.isPending ? <Loader2 className="w-3 h-3 animate-spin" /> : <Wrench className="w-3 h-3" />}
                  ...
                  <div className="space-y-1">
                    {(integrityData.issues || []).slice(0, 20).map((issue: { severity: string; message: string; autofix?: boolean }, i: number) => (
                      <div key={i} className="p-1.5 bg-lattice-deep rounded text-[10px] flex items-center gap-2">
                        ...
                        {issue.autofix && <span className="text-green-400 ml-auto">fixable</span>}
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  <div className="flex gap-2 mb-3">
                    <input
                      ...
                      onKeyDown={e => e.key === 'Enter' && wisdomQ.trim() && wisdomMutation.mutate(wisdomQ.trim())}
                    ...
                    >
                      {wisdomMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
                    </button>
                  </div>
                  {wisdomResult && (
                    <div className="p-3 bg-lattice-deep rounded-lg">
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        {Object.keys(wisdomResult.domains || {}).map(d => (
                          <span key={d} className="text-[10px] px-1.5 py-0.5 bg-neon-cyan/10 text-neon-cyan rounded">
                            {d}
                          </span>
                        ))}
                      </div>
