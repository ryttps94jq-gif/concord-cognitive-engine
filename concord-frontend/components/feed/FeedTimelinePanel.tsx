...
import { WaveformPlayer, type AudioAttachment } from '@/app/lenses/feed/WaveformPlayer';
import { subscribe } from '@/lib/realtime/socket';
import { useQuery, useMutation, useQueryClient } from '@tanstack/react-query';
import { useFeedPosts, type FeedTab } from '@/components/feed/useFeedPosts';
...
const _pickGrad = (i: number) => gradients[i % gradients.length];
...
export function FeedTimelinePanel({
  ...
  const {
    ...
    hasNextPage,
    isFetchingNextPage,
  } = useFeedPosts(tab, fallbackPosts);
  ...
  const filteredPosts = useMemo(() => {
    if (!feedPosts) return [];
    let posts = feedPosts;
    if (tab === 'releases') posts = posts.filter((p) => p.type === 'release');
    if (tab === 'trending') posts = [...posts].sort((a, b) => b.views - a.views);
    if (searchQuery.trim()) {
    ...
    <div className="flex-1 min-w-0 flex">
      ...
      <main className="flex-1 max-w-2xl border-r border-lattice-border">
        <header className="sticky top-0 z-10 bg-lattice-bg/80 backdrop-blur-md border-b border-lattice-border px-4 py-3">
          <div className="flex items-center justify-between">
            <h2 className="text-lg font-bold text-white capitalize">{tab.replace('-', ' ')}</h2>
