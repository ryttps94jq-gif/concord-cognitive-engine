...
describe('hash computation throughput', () => {
  ...
  it('CRC32 of 10000 payloads (48B each) in under 200ms', () => {
...
describe('key generation time', () => {
  it('generates 100 Ed25519 keypairs in under 500ms', async () => {
    const ms = await measureMsAsync(async () => { for (let i = 0; i < 100; i++) await mockCrypto.ed25519GenerateKeypair(); });
    expect(ms).toBeLessThan(500);
  });

  it('generates 10000 random ID strings in under 500ms', () => {
    const ms = measureMs(() => { for (let i = 0; i < 10000; i++) generateId('dtu'); });
    expect(ms).toBeLessThan(500);
  });

  it('generates 1000 random byte arrays (32B) in under 100ms', () => {
...
describe('signature creation/verification throughput', () => {
  it('signs 500 messages in under 500ms', async () => {
  ...
  it('verifies 500 signatures in under 500ms', async () => {
    const msg = new Uint8Array(256).fill(0xcc);
  ...
  it('constant-time comparison 10000 matching pairs in under 100ms', () => {
  ...
  it('constant-time comparison 10000 mismatched pairs in under 100ms', () => {
